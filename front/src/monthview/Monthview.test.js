...
describe('testing monthview', () => {
  ...
  it('should render monthView when URL is broken/invalid', async () => {
    utils.getSchedulingFreeform = jest.fn(() => ({ month: mockMonth('09', '28') }));
    ...
    await waitFor(() => expect(screen.getByText('October, 2020'))
      .toBeInTheDocument());
  });
  ...
  it('should change to previous, current and next month', async () => {
    ...
    await waitFor(() => expect(screen.getByText('October, 2020'))
      .toBeInTheDocument());
    utils.getSchedulingFreeform = jest.fn(() => ({ month: mockMonth('08', '31') }));
    fireEvent.click(screen.getByTestId('previousMonth'));
    await waitFor(() => expect(
      screen.getByText('September, 2020'),
    ).toBeInTheDocument());
    utils.getSchedulingFreeform = jest.fn(() => ({ month: mockMonth('09', '28') }));
    ...
    utils.getSchedulingFreeform = jest.fn(() => ({ month: mockMonth('10', '26') }));
    fireEvent.click(screen.getByTestId('nextMonth'));
    await waitFor(() => expect(
      screen.getByText('November, 2020'),
    ).toBeInTheDocument());
  });
});
