...
export function LinguiProvider({
  children,
  defaultLang = 'fi',
}: { children: React.ReactNode; defaultLang?: string }) {
  const langState = useState(
    LANGUAGES.includes(localStorageLang) ? localStorageLang : defaultLang,
  );
  ...
  useEffect(() => {
    (async () => {
      const { messages } = await import(
        `./locales/${lang.toLowerCase()}/messages.po`
      );
  ...
  return (
    <languageContext.Provider
      value={
        langState as [Language, React.Dispatch<React.SetStateAction<Language>>]
      }
    >
