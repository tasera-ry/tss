...
function InfoMessageCard({ message }) {
  ...
  const deleteMessage = async () => {
    const confirm = window.confirm(t`Are you sure you want to delete this message?`);
    if (confirm) {
  ...
  const start = useMemo(() => {
    return new Date(message.start).toLocaleDateString('fi-FI', { year: 'numeric', month: 'long', day: 'numeric' });
  }, [message.start]);

  const end = useMemo(() => {
    return new Date(message.end).toLocaleDateString('fi-FI', { year: 'numeric', month: 'long', day: 'numeric' });
  }, [message.end]);
