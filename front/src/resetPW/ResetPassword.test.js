...
describe('testing ResetPassword component', () => {
  it('should render ResetPassword', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Input your e-mail address in order to reset your password')).toBeInTheDocument());
  });
  ...
  it('should give error on empty field', async () => {
    await act(async () => {
      ...
      await waitFor(() => expect(screen.getByTestId('emailField')).toBeInTheDocument());
      userEvent.type(screen.getByTestId('emailField'), '');
      fireEvent.click(screen.getByText('Reset'));
      await waitFor(() => expect(screen.getByText('E-mail address is required!')).toBeInTheDocument());
    });
  });
  ...
  it('should give error on wrong email address', async () => {
    await act(async () => {
      ...
      fireEvent.click(screen.getByText('Reset'));
      await setTimeout(() => expect(screen.getByText('Invalid e-mail address!')).toBeInTheDocument(), 5000);
