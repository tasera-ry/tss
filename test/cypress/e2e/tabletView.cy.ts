...
describe.skip('Tablet view actions as superuser', () => {
  ...
  it('Close and open the center in tablet view', () => {
    cy.get('[dataid=rangeOfficerStatus]')
      .invoke('text')
      .then((text) => {
        if (text !== 'Closed') {
    ...
    cy.get('[dataid=rangeOfficerStatus]')
      .invoke('text')
      .should('eq', 'Range officer present');
    cy.get('[data-testid="trackSupervisorButton"]')
      .first()
  ...
  it('Add a visitor', () => {
    ...
    cy.get('[name="amount-of-visitors"]')
      .first()
      .invoke('text')
      .then((text) => {
