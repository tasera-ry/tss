...
exports.seed = async function (knex) {
	const schedule = await knex('track')
		...
		.whereNotNull('scheduled_range_supervision.association_id');

	const generateSupervisions = Promise.all(
		schedule.map(({ trackId, supervisionId }) => casual.track_supervision(trackId, supervisionId))
	);

	ora.promise(generateSupervisions, `Generating ${schedule.length} track supervisions`);

	const supervisions = await generateSupervisions;

	const insertSupervisions = Promise.all(
		_.chunk(supervisions, config.seeds.chunkSize).map(
			async (supervisionChunk) => await knex('track_supervision').insert(supervisionChunk)
		)
	);

	ora.promise(insertSupervisions, 'Inserting track supervisions');
