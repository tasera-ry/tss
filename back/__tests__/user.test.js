...
describe(`${endpoint}`, () => {
  ...
  describe('POST', () => {
    ...
    it('When token bearer is rangeofficer: returns code 403 and doesnt create user', async () => {
      await userModel.create({
        name: 'normal',
        role: 'rangeofficer',
        id: 'id223',
    ...
    it('When token bearer is superuser or association: returns code 201 and creates user', async () => {
      await userModel.create({
        name: 'admin',
        role: 'superuser',
        id: 'id123',
      ...
      expect(createdUser).toEqual({ name: 'normal3', role: 'association' });
    });
  });
});
